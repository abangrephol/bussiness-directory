...
<div class="form-group {{ $errors->has($id)?'has-error':'' }}">
    {{ Form::label($id, $label, array('class' => (isset($singleRow)?'col-sm-2':(isset($right)?'':'col-sm-offset-1 ').'col-sm-3').' control-label '.(isset($required)?($required ? 'required' : '' ):'') )) }}
    <div class="{{ (isset($singleRow)?'col-sm-9':'col-sm-7') }}">
        {{ Form::select($id,isset($value)?$value:array(),isset($selected)?$selected:'',array('class'=>'form-control',!isset($multiple)?:'multiple','data-placeholder'=>isset($placeholder)?$placeholder:'',isset($readonly)? 'readonly': 'noronly' ,isset($required)?($required ? 'required' : 'norequired' ):'norequired')) }}
